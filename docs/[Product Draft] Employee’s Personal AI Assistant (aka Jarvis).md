...
5\. Public Sector (National institutes, Ministries, Defence, etc) \-- \[high \#deals for Employee Service\]

**Why these:** Salesforce publicly invests heavily in industry-specific solutions for these verticals, which is a strong signal of strategic revenue importance. Public installed-base data shows Salesforce is broadly strongest in large enterprise-heavy sectors & Enlyft’s public CRM usage data shows Salesforce’s largest visible segments include IT/services and software  
And we are seeing high \#deals for Employee Service product in Public Sector 
...
| Skills/Behavior authoring | An example was provided where an employee defines a behavior to **delete junk email**. **Jarvis/A3 Orchestration Role:** Jarvis manages the plan execution, which may involve multi-step sequences such as identifying junk email, performing the deletion, and delivering a confirmation summary. **Agentforce Unit Work Role:** Every individual step or "unit work" must be supported by an existing **Agentforce action**. For instance, the specific action to "delete the email" would require a dedicated Agentforce Action, potentially utilizing a Gmail connector.  |  |  | Potential Implications of Missing Unit Actions If a prerequisite Agentforce unit action has not been deployed, the high-level behavior defined by the employee will fail due to the lack of an executable path for that specific step. In the absence of a required action, the system may attempt to locate alternatives, such as querying Google Calendar. **Critical Dependency:** The availability of corresponding unit actions is mandatory for the successful execution of any employee-defined skill. |
| Licensing/permission management strategy?  |  |  |  |  |
| Inputs from Alex 28th April notes |  |  |  |  |
| Discussion on synchronised and asynchronous data flows \- and risk of “async” approach for “Important use cases like urgent email, deadline, incident, etc”  |  |  |  | Discuss with Bikram approach where it will be realtime data read/write/updates vs async ingestion (async updates?)What about cases where DC connectors do not exist? how  can anypoint connector be used in combo with DC? |
|  |  |  |  |  |
