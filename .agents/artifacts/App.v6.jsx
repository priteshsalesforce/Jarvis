...
import {
  ...
  BrainCircuit, Zap, TrendingUp, FileText, MessageSquare, Lock,
...
function TimelinePanel({ onEventClick, onAddMeeting }) {
  ...
  // Simulated "now" = 9:45 AM
  const nowMinutes = 9*60+45
  const toMin = t => { const [h,m]=t.split(':').map(Number); return h*60+m }
  return (
    <div style={{ width:280, flexShrink:0, borderLeft:`1px solid ${T.border}`, overflowY:'auto', padding:'20px 14px', background:T.rail }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <div>
          <p style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.12em', color:T.textXsoft }}>Today's schedule</p>
          <p style={{ fontSize:14, fontWeight:800, color:T.text, marginTop:2 }}>Thursday, May 1</p>
        </div>
        <Btn variant="secondary" icon={Plus} onClick={() => { SFX.tap(); onAddMeeting() }}
          style={{ padding:'5px 10px', fontSize:11 }}>Add</Btn>
      </div>
      <div style={{ position:'relative' }}>
        {/* vertical guide */}
        <div style={{ position:'absolute', left:32, top:0, bottom:0, width:1, background:T.border }} />
        {TODAY_EVENTS.map((ev, i) => {
          ...
          // "Now" line between past and upcoming
          const showNowLine = i > 0 && toMin(TODAY_EVENTS[i-1].time) < nowMinutes && evMin >= nowMinutes
          return (
            <div key={ev.id}>
              {showNowLine && (
                <div style={{ display:'flex', alignItems:'center', gap:6, margin:'8px 0', paddingLeft:22 }}>
                  <div style={{ width:20, height:20, borderRadius:'50%', flexShrink:0, background:T.red,
                    display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 0 8px ${T.red}50`, zIndex:1, position:'relative' }}>
              ...
              <div onClick={() => { SFX.tap(); HX.tap(); onEventClick(ev) }}
                style={{ display:'flex', gap:8, marginBottom:8, cursor:'pointer', paddingLeft:2, opacity:isPast?.5:1, transition:'opacity .2s' }}>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', width:44, flexShrink:0 }}>
                  <span style={{ fontSize:10, fontWeight:600, color:T.textSoft, marginBottom:4 }}>{ev.time}</span>
                  <div style={{ width:10, height:10, borderRadius:'50%', background:isPast?T.border:color,
                    boxShadow:isNow?`0 0 8px ${color}80`:'none', zIndex:1, position:'relative',
                    transition:'background .3s, box-shadow .3s' }} />
                </div>
                <div style={{ flex:1, padding:'9px 11px', borderRadius:12,
                  background:isNow?`${T.core}08`:T.surface,
                  border:`1px solid ${isNow?`${T.core}30`:T.border}`,
                  transition:'all .2s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor=`${T.core}40` }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor=isNow?`${T.core}30`:T.border }}>
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:4 }}>
                    <p style={{ fontSize:12, fontWeight:700, color:T.text, lineHeight:1.3, flex:1 }}>{ev.title}</p>
                    {ev.hasPrep && <Chip text="Prep ✓" color={T.blue} />}
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:4 }}>
                    <div style={{ width:5, height:5, borderRadius:'50%', background:color, flexShrink:0 }} />
                    <span style={{ fontSize:10, color:T.textSoft }}>{ev.location}</span>
                    {ev.attendees.length > 0 && (
                      <span style={{ fontSize:10, color:T.textXsoft }}>· {ev.attendees[0]}{ev.attendees.length>1?` +${ev.attendees.length-1}`:''}</span>
                    )}
                  </div>
                </div>
              </div>
    ...
    <div style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
      ...
      <div className="float-anim" style={{ position:'relative', width:100, height:100, marginBottom:28 }}>
        ...
        <div style={{ width:100, height:100, borderRadius:28, display:'flex', alignItems:'center', justifyContent:'center',
          ...
          <BrainCircuit size={44} color="white" />
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, width:'100%', maxWidth:520, marginBottom:28 }}>
        {[
          ...
          { Icon:BrainCircuit, color:T.amber, title:'Anticipate', desc:'Detects what\'s missing or about to matter.' },
...
function PersonaliseView({ onNew }) {
  ...
  return (
    <div style={{ padding:'24px 24px', overflowY:'auto', height:'100%' }}>
      <h2 style={{ fontSize:18, fontWeight:800, color:T.text, marginBottom:4 }}>Make Jarvis yours</h2>
      <p style={{ fontSize:12, color:T.textSoft, marginBottom:24 }}>What it watches, when it acts, which systems it can touch.</p>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
            <p style={{ fontSize:13, fontWeight:700, color:T.text }}>Behaviours</p>
            <Btn variant="primary" icon={Plus} onClick={() => { SFX.tap(); onNew() }} style={{ padding:'5px 12px', fontSize:11 }}>New</Btn>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {BEHAVIOURS.map(b => (
              <GlassCard key={b.id} style={{ padding:'12px 14px', opacity:b.enabled?1:.55 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <div style={{ flex:1 }}>
                    <p style={{ fontSize:12, fontWeight:600, color:T.text }}>{b.name}</p>
                    <p style={{ fontSize:10, marginTop:2, color:T.textSoft }}>{b.schedule}</p>
                  </div>
                  <Toggle value={b.enabled} onChange={() => SFX.tap()} />
                </div>
              </GlassCard>
            ))}
          </div>
        ...
        <div>
          <p style={{ fontSize:13, fontWeight:700, color:T.text, marginBottom:12 }}>Connections</p>
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {conns.map(c => (
              <GlassCard key={c.id} style={{ padding:'10px 14px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
