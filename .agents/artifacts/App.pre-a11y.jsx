...
.expand-down { animation: expandDown .2s cubic-bezier(.16,1,.3,1) both; }
...
const INTENTS = [
  ...
  { id:'p2', tier:'L2', cat:'Pending with you',
    ...
    evidence:'Finance: Approved · IT Security: SLA +5 days', chatScenario:'diskspace' },
]
...
const MANAGER_INTENTS = [
  ...
  { id:'m1', tier:'L2', cat:'Approval digest',
    ...
    evidence:'All 3 policy-checked · Exception: salary +11% flagged separately', chatScenario:'approvals' },
    ...
    <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginTop:10 }}>
      {actions.map(a => (
        <button key={a.key} type="button"
          ...
          style={{ fontSize:11, fontWeight:600, padding:'5px 12px', borderRadius:99, cursor:'pointer',
            background:T.surface, color:T.text, border:`1px solid ${T.borderMid}`,
...
function AgentTrace({ trace }) {
  ...
  return (
    <div style={{ borderRadius:10, background:T.coreSoft, border:`1px solid ${T.core}20`, marginTop:8, overflow:'hidden' }}>
      {/* Header row */}
      <button type="button" onClick={() => { SFX.tap(); setOpen(o=>!o) }}
        ...
        <span style={{ flex:1, fontSize:11, fontWeight:600, color:T.core, lineHeight:1.4 }}>{trace.summary}</span>
          ...
          {trace.steps.map((step, si) => (
            <div key={si} style={{ marginTop:8 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:step.bullets?.length?4:0 }}>
                ...
                <span style={{ fontSize:11, fontWeight:600, color:T.text, flex:1 }}>{step.label}</span>
                {step.plugin && (
                  <span style={{ fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:6,
                    background:`${T.core}15`, color:T.core, border:`1px solid ${T.core}25`,
              ...
              {step.bullets?.map((b,bi) => (
                <div key={bi} style={{ display:'flex', alignItems:'flex-start', gap:5, paddingLeft:22, marginTop:2 }}>
                  <span style={{ color:T.textXsoft, fontSize:10, marginTop:1 }}>·</span>
                  <span style={{ fontSize:10, color:T.textSoft, lineHeight:1.5 }}>{b}</span>
                </div>
              ))}
            </div>
          ))}
  ...
  return (
    <div style={{ display:'flex', gap:6, marginTop:10, overflowX:'auto', paddingBottom:2 }}>
      {sources.map((s, i) => {
        const Icon = s.icon
        return (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 9px', borderRadius:9, flexShrink:0,
            ...
            <div style={{ minWidth:0 }}>
              <p style={{ fontSize:10, fontWeight:700, color:T.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:110 }}>{s.name}</p>
              <p style={{ fontSize:9, color:T.textXsoft, whiteSpace:'nowrap' }}>{s.updated}</p>
            </div>
          </div>
        )
      })}
...
function MessageTable({ table }) {
  const T = window.__T
  return (
    <div style={{ overflowX:'auto', marginTop:10, borderRadius:10, border:`1px solid ${T.border}` }}>
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11.5 }}>
        <thead>
          <tr>
            {table.headers.map((h,i) => (
              <th key={i} style={{ padding:'7px 10px', textAlign:'left', fontWeight:700,
                background:T.coreSoft, color:T.core, fontSize:10, textTransform:'uppercase', letterSpacing:'0.05em',
...
function AgentFlowPanel({ conv }) {
  ...
  return (
    <div style={{ width:252, flexShrink:0, borderLeft:`1px solid ${T.border}`, overflowY:'auto',
      ...
      <p style={{ fontSize:10, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.15em',
        ...
        <div key={i} style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
          <div style={{ width:'100%', padding:'10px 12px', borderRadius:12,
            ...
            <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:11.5, fontWeight:600, color:T.text, lineHeight:1.4 }}>{step.label}</p>
                {step.plugin && (
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginTop:0 }}>
        ...
        <p style={{ fontSize:10, fontWeight:600, color:T.green, marginTop:6 }}>Completed</p>
      </div>
    </div>
  )
}
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:T.appBg, fontFamily:T.font, position:'relative', transition:'background .3s' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, overflow:'hidden', position:'relative', zIndex:1 }}>
        ...
        <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          ...
          {scene==='app' && tab==='today' && (
            <div style={{ flex:1, overflowY:'auto', padding:'24px 0 100px' }}>
              <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 24px' }}>
                ...
                <div style={{ display:'flex', alignItems:'flex-start', gap:24 }}>
                  ...
                  {/* Right: schedule + feed — flows naturally, scrolls with page */}
